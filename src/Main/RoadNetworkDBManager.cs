  ...
  public class RoadNetworkDBManager
  {
    ...
    public RoadNetworkDBManager(RoadNetworkDBManagerInput input)
    {
      ...
      if (myInput.MyTraceSource != null)
        myInput.MyTraceSource.TraceEvent(TraceEventType.Information, (int)ProcessEvents.Started, DateTime.Now.ToShortDateString() + " " + DateTime.Now.ToShortTimeString() + ": RoadNetworkManager Engine Loaded");
    }
    ...
    public DataTable ListAvailableDatabases()
    {
      ...
      DataTable list = null;
      string selectcomand = "select associatedTableName as Name, provider + ' ' + providerversion AS Provider, dateAdded, [description] as Comment, isPrimary, recordCount, isErr from " + myInput.MasterTableName + " order by dateAdded desc";
    ...
    public void DeleteDatabase()
    {
      ...
      try
      {
        ...
        removeCommand.ExecuteNonQuery();
        // -------
        removeCommand.CommandText = "delete from " + myInput.MasterTableName + " where associatedTableName = '" + myInput.DatabaseToModify + "'";
        removeCommand.ExecuteNonQuery();
        trans.Commit();
      }
      catch (Exception ex)
    ...
    public string MergeDataBasesValidate()
    {
      try
      {
        ...
        validCommand.CommandText = "select count(*) from " + myInput.MasterTableName + " where (isErr <> 0) and (" + whereCluase + ")";
        if ((int)(validCommand.ExecuteScalar()) != 0) return "One of the databases is corrupted and cannot be merged.";
        validCommand.CommandText = "select count(distinct(provider + ' ' + providerversion)) from " + myInput.MasterTableName + " where (" + whereCluase + ")";
    ...
    public void MergeDatabases()
    {
      ...
      checkCommand.CommandText = "select count(*) from " + myInput.MasterTableName + " where (isPrimary = 1) and (" + whereCluase + ")";
      isPrimery = ((int)(checkCommand.ExecuteScalar()) > 0);
      checkCommand.CommandText = "select max([dateAdded]) from " + myInput.MasterTableName + " where (" + whereCluase + ")";
      ...
      try
      {
        ...
        for (int i = 1; i < myInput.DatabasesToMerge.Count; i++)
        {
          ...
          mergeCommand.ExecuteNonQuery();
          mergeCommand.CommandText = "delete from " + myInput.MasterTableName + " where associatedTableName = '" + myInput.DatabasesToMerge[i] + "'";
          mergeCommand.ExecuteNonQuery();
        }
        if (isPrimery)
        {
          mergeCommand.CommandText = "UPDATE " + myInput.MasterTableName + " SET isPrimary = 1  WHERE associatedTableName = '" + myInput.DatabasesToMerge[0] + "'";
          mergeCommand.ExecuteNonQuery();
        }
        try
        {
        ...
        newCount += (int)(checkCommand.ExecuteScalar());
        checkCommand.CommandText = "UPDATE " + myInput.MasterTableName + " SET recordCount = " + newCount + "  WHERE associatedTableName = '" + myInput.DatabasesToMerge[0] + "'";
        checkCommand.ExecuteNonQuery();
        checkCommand.CommandText = "UPDATE " + myInput.MasterTableName + " SET [dateAdded] = '" + dateAdded + "'  WHERE associatedTableName = '" + myInput.DatabasesToMerge[0] + "'";
        checkCommand.ExecuteNonQuery();
        checkCommand.CommandText = "UPDATE " + myInput.MasterTableName + " SET [isErr] = " + 0 + "  WHERE associatedTableName = '" + myInput.DatabasesToMerge[0] + "'";
        checkCommand.ExecuteNonQuery();
      }
      catch (Exception ex)
      {
    ...
    public void MakeDatabasePrimary()
    {
      ...
      try
      {
        isPrimaryCommand.CommandText = "UPDATE " + myInput.MasterTableName + " SET isPrimary = 0  WHERE isPrimary = 1";
        isPrimaryCommand.ExecuteNonQuery();
        isPrimaryCommand.CommandText = "UPDATE " + myInput.MasterTableName + " SET isPrimary = 1  WHERE associatedTableName = '" + myInput.DatabaseToModify + "'";
        isPrimaryCommand.ExecuteNonQuery();
        trans.Commit();
      }
      catch (Exception ex)
    ...
    private void myWorker_DoWork(object sender, DoWorkEventArgs e)
    {
      ...
      if (input.ImporterInput.MyDataProvider == DataProvider.Navteq)
      {
        ...
        try
        {
          ...
          addCommand.ExecuteNonQuery();
          addCommand.CommandText = "INSERT INTO " + input.MasterTableName +
    ...
    private void myWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
    {
      ...
      try
      {
        finishCommand.CommandText =
          "UPDATE " + myInput.MasterTableName + " SET recordCount = " + o.ProcessedInsertCount + "  WHERE associatedTableName = '" + myInput.ImporterInput.RoadNetworkDatabaseName + "'";
        finishCommand.ExecuteNonQuery();
        if (!o.Cancelled)
        {
          finishCommand.CommandText =
            "UPDATE " + myInput.MasterTableName + " SET isErr = " + o.ErrorCount + "  WHERE associatedTableName = '" + myInput.ImporterInput.RoadNetworkDatabaseName + "'";
          finishCommand.ExecuteNonQuery();
          if (myInput.ImporterInput.SetAsPrimary)
          {
            finishCommand.CommandText = "UPDATE " + myInput.MasterTableName + " SET isPrimary = 0  WHERE isPrimary = 1";
            finishCommand.ExecuteNonQuery();
            finishCommand.CommandText =
              "UPDATE " + myInput.MasterTableName + " SET isPrimary = 1  WHERE associatedTableName = '" + myInput.ImporterInput.RoadNetworkDatabaseName + "'";
            finishCommand.ExecuteNonQuery();
          }
        }
        trans.Commit();
    ...
    public List<string> ListAvailableDatabaseNames()
    {
      ...
      try
      {
        ...
        select.CommandText = "select associatedTableName from " + myInput.MasterTableName;
    ...
    {
      bool ret = false;
      if (myInput.MasterTableName == string.Empty) myInput.MasterTableName = "AvailableRoadNetworkData";
      var con = new SqlConnection(myInput.SQLConnectionString);
      try
      {
        con.Open();
      ...
      try
      {
        if (con.State == ConnectionState.Open)
        {
          var checkCommand = con.CreateCommand();
          checkCommand.Connection = con;
          checkCommand.CommandText = "SELECT [associatedTableName],[isPrimary],[description],[provider],[providerVersion],[dateAdded],[recordCount],[isErr] FROM " + myInput.MasterTableName + " WHERE [isPrimary] = 1 and [isErr] = 0";
          checkCommand.ExecuteNonQuery();
        }
        ret = true;
      }
    ...
    public string SetupDatabase()
    {
      ...
      try
      {
        ...
        setupCommand.CommandText = "IF OBJECT_ID('" + myInput.MasterTableName + "', 'U') IS NOT NULL DROP TABLE " + myInput.MasterTableName;
        setupCommand.ExecuteNonQuery();
        setupCommand.CommandText =
            "CREATE TABLE [dbo].[" + myInput.MasterTableName + "](" + Environment.NewLine +
            ...
            "[recordCount] [int] NULL,CONSTRAINT [PK_" + myInput.MasterTableName + "] PRIMARY KEY CLUSTERED" + Environment.NewLine +
      ...
      finally
      {
        if (con.State == ConnectionState.Open) con.Close();
        if (myInput.MyTraceSource != null)
          myInput.MyTraceSource.TraceEvent(TraceEventType.Verbose, (int)FunctionEvents.Exit, DateTime.Now.ToLongTimeString() + ": SetupDatabase ended. result: " + ret);
      }
      return ret;
    }
  }
